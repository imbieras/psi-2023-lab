...
public interface IUserService
{
    ...
    Task<List<Hobby>?> GetHobbiesById(UserId userId);
    Task<IUser?> GetUserByUsernameAsync(string username);
    Task BanUserAsync(UserId userId);
    Task<UserId> RegisterUserAsync(string username, UserFlags flags, UserTraits traits, List<Hobby> hobbies);
    ...
    Task AddHobbiesToUserAsync(UserId userid, List<Hobby> hobbies);
}
